...
public class MunicipioServiceImpl implements MunicipioService {
	...
	public void rimuovi(Municipio municipioInstance) {
		if (municipioDAO.findOneEagerAbitanti(municipioInstance.getId()).getAbitanti().isEmpty()) {
			municipioDAO.delete(municipioInstance);
		} else {
			throw new RuntimeException("impossibile eliminare il municipio, perchè contiene abitanti");
		}
	}
	...
	@Transactional(readOnly = true)
	public Municipio caricaMunicipioEagerAbitanti(Long id) {
		return municipioDAO.findOneEagerAbitanti(id);
	}
}
