...
public class MunicipioDAOImpl implements MunicipioDAO {
	...
	public List<Municipio> findByExample(Municipio example) {
		...
		if (StringUtils.isNotEmpty(example.getUbicazione())) {
			whereClauses.add(" m.ubucazione like :ubucazione ");
			paramaterMap.put("ubucazione", "%" + example.getUbicazione() + "%");
		}
	...
	public Municipio findOneEagerAbitanti(Long id) {
		TypedQuery<Municipio> query = entityManager
				.createQuery("from Municipio m left join fetch m.abitanti a where m.id = ?1", Municipio.class);
